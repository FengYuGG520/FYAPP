...
@implementation FYClassIvarInfo
...
@implementation FYClassMethodInfo
...
@implementation FYClassPropertyInfo
...
- (void)_update {
    ...
    if (methods) {
        ...
        for (unsigned int i = 0; i < methodCount; i++) {
            FYClassMethodInfo *info = [[FYClassMethodInfo alloc] initWithMethod:methods[i]];
    ...
    if (properties) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            FYClassPropertyInfo *info = [[FYClassPropertyInfo alloc] initWithProperty:properties[i]];
    ...
    if (ivars) {
        ...
        for (unsigned int i = 0; i < ivarCount; i++) {
            FYClassIvarInfo *info = [[FYClassIvarInfo alloc] initWithIvar:ivars[i]];
...
+ (instancetype)classInfoWithClass:(Class)cls {
    ...
    FYClassInfo *info = CFDictionaryGetValue(class_isMetaClass(cls) ? metaCache : classCache, (__bridge const void *)(cls));
    if (info && info->_needUpdate) {
        [info _update];
    }
    dispatch_semaphore_signal(lock);
    if (!info) {
        info = [[FYClassInfo alloc] initWithClass:cls];
        if (info) {
            dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
