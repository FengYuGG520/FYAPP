...
@implementation NSLayoutConstraint (FYDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
                           @(FYLayoutPriorityDefaultHigh)      : @"high",
                           @(FYLayoutPriorityDefaultLow)       : @"low",
                           @(FYLayoutPriorityDefaultMedium)    : @"medium",
                           @(FYLayoutPriorityRequired)         : @"required",
                           @(FYLayoutPriorityFittingSizeLevel) : @"fitting size",
                           };
#elif TARGET_OS_MAC
        descriptionMap = @{
                           @(FYLayoutPriorityDefaultHigh)                 : @"high",
                           @(FYLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
                           @(FYLayoutPriorityDefaultMedium)               : @"medium",
                           @(FYLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
                           @(FYLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
                           @(FYLayoutPriorityDefaultLow)                  : @"low",
                           @(FYLayoutPriorityFittingSizeCompression)      : @"fitting size",
                           @(FYLayoutPriorityRequired)                    : @"required",
                           };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(fy_key)] && [obj fy_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj fy_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != FYLayoutPriorityRequired) {
